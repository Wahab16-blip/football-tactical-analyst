...
def save_report(match_context, report, timestamp):
    """Saves report to reports.json"""
    reports = []
...
if page == "New Report":
    ...
    squad = load_squad()   # load squad ONCE at the top
            ...
            for pos in pos_order:
                ...
                if pos_players:
                    ...
                    for p in pos_players:
                        ...
                        with c4:
                            if st.button("🗑️",
                                         key=f"sm_del_{p['name']}"):
                                squad = [x for x in squad
                                         if x["name"] != p["name"]]
                                save_squad(squad)
                                st.rerun()
